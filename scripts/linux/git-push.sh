...
#
# Usage:
#   ./scripts/linux/git-push.sh "commit message"
#   ./scripts/linux/git-push.sh                    (opens editor for commit message)
...
if [ "$THEME_CHANGED" = true ]; then
    ...
    # Stage updated submodule reference
    git -C "$REPO" add theme_smartmind
fi

# --- Commit and push plugin to dev branch ---
...
# --- Sync to Moodle ---
echo ""
echo "=== Syncing to Moodle ==="
bash "$REPO/scripts/linux/sync-theme.sh"
