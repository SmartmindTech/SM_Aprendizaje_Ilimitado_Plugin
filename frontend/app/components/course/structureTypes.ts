...
export const MOD_META: Record<string, { icon: string; label: string }> = {
  ...
  quiz:        { icon: 'bi-question-circle',   label: 'Cuestionario' },
  assign:      { icon: 'bi-pencil-square',     label: 'Tarea' },
  resource:    { icon: 'bi-file-earmark-arrow-up', label: 'Archivo' },
  ...
  wiki:        { icon: 'bi-globe',              label: 'Wiki' },
  choice:      { icon: 'bi-check2-circle',     label: 'Consulta' },
  feedback:    { icon: 'bi-chat-dots',         label: 'Retroalimentación' },
  lesson:      { icon: 'bi-mortarboard',       label: 'Lección' },
  workshop:    { icon: 'bi-people',             label: 'Taller' },
  h5pactivity: { icon: 'bi-play-circle',       label: 'Actividad H5P' },
  survey:      { icon: 'bi-clipboard-data',    label: 'Encuesta predefinida' },
  data:        { icon: 'bi-database',          label: 'Base de datos' },
  chat:        { icon: 'bi-send',              label: 'Chat' },
  bigbluebuttonbn: { icon: 'bi-camera-video',  label: 'BigBlueButton' },
  lti:         { icon: 'bi-box-arrow-up-right', label: 'Herramienta ext.' },
  imscp:       { icon: 'bi-archive',           label: 'Paquete IMS' },
  iomadcertificate: { icon: 'bi-award',        label: 'Certificado' },
  trainingevent: { icon: 'bi-calendar-event',  label: 'Evento' },
